...
const double Xmax = 100.0;                // Control x dimensions of viewable scene
const double Ymax = 60.0;                // Control y dimensions of viewable scene
const double Zmin = -120, Zmax = 120.0;   // Control z dimensions of the viewable scene

// zDistance equals the initial distance from the camera to the z = Zmax plane
const double zDistance = 140.0;              // Make this value larger or smaller to affect field of view.

double ZextraDistance = -139.0;              // Extra distance we have moved to/from the scene (-10 previously)
double ZextraDelta = 0.5;                // Pressing HOME/END moves closer/farther by this amount
const double ZextraDistanceMin = -139.0;
const double ZextraDistanceMax = 100.0;
...
void mySetViewMatrix() {
    ...
    viewMatrix.Set_glTranslate(changex + 0.0, changey + 0.0, changez-(Zmax + zDistance - 139));      // Translate to be in front of the camera // 89 previously be ZextraDistance
...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "Project 6 (student)", NULL, NULL);
	if (window == NULL) {
    ...
    printf("Press 'Q' key to toggle viewing all the objects besides the floor.\n");

...
uniform bool applyTexture;     // Set true if the function applyTextureFunction() is to be called
// uniform sampler2D theTextureMap; // Declared as needed in applyTextureFunction()
            ...
            if (dotEllNormal > 0 ) { 
                ...
                if ( !Lights[i].IsSpotLight || spotCosine > Lights[i].SpotCosCutoff ) {
                    ...
                    if ( EnableSpecular ) { 
						...
						float specDotProduct;			// Dot product for base of specFactor (specular factor)
                        if ( UseHalfwayVector ) {
                            vec3 hVector = normalize(ellVector+vVector);
							specDotProduct = dot(hVector,mvNormal);   // h dot v
                        }
						else {
                            vec3 rVector = 2.0*dotEllNormal*mvNormal - ellVector;
                            specDotProduct = dot(vVector, rVector);  // r dot v 
                        }
						if ( specDotProduct>0.0 ) {
						    specFactor = pow( specDotProduct, matSpecExponent );
						}
